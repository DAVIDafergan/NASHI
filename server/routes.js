...
router.post('/personality', authenticate, isAdmin, async (req, res) => {
    try {
        ...
        if (!p) {
            p = new Personality(req.body);
        } else {
            p.name = req.body.name;
            p.role = req.body.role;
            p.image = req.body.image;
            p.questions = req.body.questions;
...
router.post('/events/:id/join', authenticate, async (req, res) => {
    try {
        ...
        res.json({ success: true, message: `נרשמת בהצלחה! קיבלת ${config.pointsPerEventJoin} נקודות` });
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.put('/classes/:id', authenticate, isAdmin, async (req, res) => { res.json(await Class.findByIdAndUpdate(req.params.id, req.body, { new: true })); });
