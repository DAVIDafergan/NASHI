  ...
  return (
    <div className="min-h-screen pb-20 relative overflow-x-hidden font-sans text-right bg-[#fffcfc]" dir="rtl">
      ...
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
          ...
          <div className="absolute -top-24 -left-24 w-96 h-96 bg-rose-100/40 rounded-full blur-[100px] animate-blob"></div>
          <div className="absolute top-1/2 left-1/3 w-72 h-72 bg-purple-100/30 rounded-full blur-[80px] animate-blob animation-delay-2000"></div>
          <div className="absolute bottom-0 right-0 w-[400px] h-[400px] bg-purple-50/40 rounded-full blur-[100px] -mr-32 -mb-32 animate-blob animation-delay-4000"></div>
      </div>

      <div className="max-w-5xl mx-auto px-4 md:px-8 pt-4 md:pt-8 relative z-10 space-y-6 md:space-y-10">
        
        {renderAdBanner()}

        {/* הגרלות בנייד */}
        <div className="md:hidden mx-1">
          <Link to="/lottery" className="flex items-center justify-between bg-gradient-to-r from-purple-500/90 to-rose-500/90 backdrop-blur-md p-4 rounded-2xl text-white shadow-xl">
        ...
        <div className="mx-1 hidden md:block">
          {user?.isMemberApproved ? (
              <div className="bg-white/60 backdrop-blur-md p-3 md:p-4 rounded-2xl md:rounded-3xl border border-rose-100/50 flex items-center justify-between shadow-sm animate-bounce-in">
                <div className="flex items-center gap-3 md:gap-4">
                    <div className="p-2 md:p-2.5 bg-rose-50 rounded-xl shadow-inner text-rose-400"><Star fill="currentColor" size={14} /></div>
                    <div>
                      <p className="text-[7px] md:text-[8px] font-black text-rose-300 uppercase tracking-widest leading-none mb-1">הניקוד שצברת</p>
                      <span className="font-black text-slate-800 text-xs md:text-xl tracking-tighter">{(user?.points || 0).toLocaleString()} <small className="text-[8px] md:text-xs opacity-40 font-bold">PTS</small></span>
                    </div>
                </div>
                <Link to="/lottery" className="bg-slate-900 text-white px-5 md:px-7 py-1.5 md:py-2.5 rounded-xl text-[8px] md:text-xs font-black hover:bg-purple-600 transition-all shadow-md active:scale-95 flex items-center gap-1.5 font-serif">כניסה להגרלות <ChevronLeft size={14}/></Link>
              </div>
          ) : (
            <div className="bg-white/70 backdrop-blur-md p-4 md:p-8 rounded-[2rem] md:rounded-[2.5rem] text-slate-800 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm border border-purple-50">
                <div className="text-center md:text-right space-y-1 md:space-y-2 relative z-10">
                    <h3 className="text-sm md:text-xl font-black flex items-center justify-center md:justify-start gap-2 text-purple-600 tracking-tight">
                        <Sparkles size={16} md:size={18} className="text-rose-400 animate-pulse" /> {user?.isMemberRequested ? 'הבקשה בטיפול' : 'ברוכה הבאה למעגל'}
                    </h3>
                    <p className="text-[10px] md:text-sm text-slate-400 font-medium max-w-md">הצטרפי לאלפי נשים וגלי עולם של תרבות והטבות.</p>
                </div>
                {!user?.isMemberRequested && (
                  <button onClick={() => user ? setShowMembershipModal(true) : onOpenLogin()} className="bg-rose-500 text-white px-6 md:px-10 py-2 md:py-3 rounded-full font-black text-[10px] md:text-sm shadow-lg hover:bg-purple-600 transition-all active:scale-95 flex items-center gap-2">
        ...
        {/* סליידר אירועים */}
        <section className="relative h-[280px] md:h-[400px] w-full overflow-hidden rounded-[2rem] md:rounded-[2.5rem] shadow-[0_30px_60px_-15px_rgba(168,85,247,0.25)] mx-0 border-0 transition-all duration-500">
            ...
            <div key={event.id || index} className={`absolute inset-0 transition-all duration-1000 ease-out ${index === currentSlide ? 'opacity-100 scale-100' : 'opacity-0 scale-105'}`}>
                ...
                <div className="absolute bottom-0 left-0 p-6 md:p-10 text-left w-full flex flex-col items-start z-20">
                    <div className="inline-flex items-center gap-1.5 bg-purple-600/90 backdrop-blur-md text-white px-3 py-1 rounded-full text-[8px] md:text-[9px] font-black uppercase mb-2 shadow-lg border border-white/20">
                      <Sparkles size={10} className="text-amber-300" /> אירוע נבחר
                    </div>
                    <h2 className="text-xl md:text-4xl font-black text-white mb-2 md:mb-4 tracking-tight drop-shadow-2xl font-serif">{event.title}</h2>
                    <div className="flex flex-wrap items-center gap-2 md:gap-4 justify-start text-white/90 font-bold mb-4 md:mb-6">
                        <p className="flex items-center gap-1.5 text-[9px] md:text-xs bg-black/30 backdrop-blur-sm px-3 py-1.5 rounded-full border border-white/10">
                    ...
                    <Link to="/events" className="inline-flex items-center gap-2 bg-white text-slate-900 px-6 md:px-8 py-2 md:py-3 rounded-2xl font-black text-[10px] md:text-xs hover:bg-purple-600 hover:text-white transition-all shadow-xl active:scale-95 group font-serif">
        ...
        <div className="flex gap-3 overflow-x-auto pb-4 no-scrollbar px-1">
            <button onClick={() => navigate('/classes')} 
                    className="flex items-center gap-2 px-5 md:px-7 py-3 md:py-3.5 bg-slate-900 rounded-2xl text-[9px] md:text-xs font-black text-white shadow-xl transition-all flex-shrink-0 active:scale-95 border-b-2 border-purple-500/50">
              <GraduationCap size={16} className="text-purple-400" /> חוגי המעגל
            </button>
            <Link to="/personality-archive" className="flex items-center gap-2 px-5 md:px-7 py-3 md:py-3.5 bg-purple-500 rounded-2xl text-[9px] md:text-xs font-black text-white shadow-xl transition-all flex-shrink-0 active:scale-95 border-b-2 border-white/20 font-serif">
              <Users size={16} className="text-purple-100" /> נשות המעגל
            </Link>
            {categories.map((cat, idx) => (
              <button key={idx} onClick={() => navigate('/events', { state: { category: cat.name } })} 
                      className="flex items-center gap-1.5 px-5 md:px-6 py-3 md:py-3.5 bg-white rounded-xl text-[9px] md:text-xs font-bold text-slate-500 shadow-sm border border-rose-50 hover:border-purple-200 transition-all flex-shrink-0 active:shadow-md">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10">
                ...
                {personality && personality.name && (
                    <section className="animate-fade-in px-1">
                        <div className="flex items-center justify-between mb-4 px-2">
                            <h3 className="text-sm md:text-lg font-black text-slate-800 tracking-tight flex items-center gap-2"><Sparkles size={18} className="text-purple-400"/> אשת השבוע</h3>
                            <Link to="/personality-archive" className="text-rose-400 font-black text-[9px] md:text-xs flex items-center gap-0.5 hover:underline font-serif">כל הראיונות <ChevronLeft size={10}/></Link>
                        </div>
                        <Link to={`/personality-archive`} className="block bg-white rounded-[2rem] p-5 md:p-7 shadow-sm border border-purple-50 hover:shadow-xl transition-all group relative overflow-hidden">
                            ...
                            <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8 relative z-10">
                                <div className="relative shrink-0">
                                    ...
                                    <img src={personality.image} className="w-24 h-24 md:w-40 md:h-40 rounded-2xl md:rounded-3xl object-cover shadow-lg border-2 border-white relative z-10" alt={personality.name} />
                                    <div className="absolute -bottom-2 -right-2 bg-amber-400 p-1.5 md:p-2 rounded-lg text-white shadow-md z-20 animate-pulse"><Quote size={14} md:size={16}/></div>
                                </div>
                                <div className="text-center md:text-right space-y-2 md:space-y-3 flex-1">
                                    <span className="text-[7px] md:text-[9px] font-black text-purple-400 uppercase tracking-widest bg-purple-50 px-3 py-1 rounded-full inline-block font-serif">Personality Spotlight</span>
                                    <div>
                                        <h3 className="text-lg md:text-2xl font-black text-slate-900 leading-tight group-hover:text-rose-600 transition-colors font-serif">{personality.name}</h3>
                                        <p className="text-[9px] md:text-xs text-slate-400 font-bold mt-0.5">{personality.role}</p>
                                    </div>
                                    <div className="relative pt-3 md:pt-4 border-t border-rose-50">
                                        <Quote size={14} md:size={18} className="text-rose-100 absolute -top-1 md:-top-2 -right-1" />
                                        <p className="text-[10px] md:text-sm text-slate-500 font-serif italic leading-relaxed line-clamp-2">
                                    ...
                                    <div className="text-[8px] md:text-xs font-black text-rose-400 group-hover:gap-2 flex items-center gap-1 transition-all pt-1">
                                        קראי את הראיון המלא <ChevronLeft size={12} />
                                    </div>
                                </div>
                            </div>
                        </Link>
                ...
                <div className="space-y-4 px-1">
                    <div className="flex items-center justify-between px-1">
                      <h3 className="text-sm md:text-lg font-black text-slate-800 flex items-center gap-2 tracking-tight">
                        <GraduationCap className="text-purple-400" size={20}/> חוגים והעשרה
                      </h3>
                        ...
                        {classes.slice(0, 4).map((cls, idx) => (
                            <div key={cls._id || idx} className="bg-white p-3 md:p-4 rounded-[1.5rem] shadow-sm border border-purple-50 flex items-center gap-4 group hover:bg-purple-50/20 transition-all cursor-pointer">
                                ...
                                <div className="text-right flex-1 overflow-hidden">
                                    <h4 className="font-black text-slate-800 text-[10px] md:text-sm group-hover:text-purple-600 transition-colors truncate mb-0.5 font-serif">{cls.title}</h4>
                                    <p className="text-slate-400 text-[8px] md:text-xs line-clamp-1 font-medium">{cls.instructor} | {cls.day}</p>
                                </div>
                    ...
                    <section className="space-y-4 animate-fade-in px-1">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-5 overflow-x-auto md:overflow-visible pb-2 no-scrollbar">
                            {communityItems.slice(0, 6).map((item, idx) => (
                                <div key={item._id || item.id || idx} className="bg-white p-2.5 md:p-4 rounded-[1.2rem] md:rounded-[2rem] shadow-sm border border-rose-50 group hover:border-purple-200 transition-all flex-shrink-0 md:flex-shrink w-44 md:w-auto">
                                    <img src={item.image} className="w-full h-24 md:h-32 rounded-xl md:rounded-2xl object-cover mb-2 group-hover:opacity-90 transition-opacity" />
                                    <div className="text-right px-1">
                                        <span className="text-[6px] md:text-[8px] font-black text-purple-300 uppercase block mb-0.5 font-serif">{item.category}</span>
                                        <h4 className="font-black text-slate-800 text-[10px] md:text-xs truncate leading-tight font-serif">{item.title}</h4>
            ...
            <div className="space-y-6 md:space-y-8 px-1 md:px-0">
                ...
                {/* השראה יומית מעודכן */}
                <div className="bg-slate-900 rounded-[2.5rem] p-6 md:p-8 text-white relative overflow-hidden shadow-2xl text-right group">
                    ...
                    <div className="relative z-10 space-y-4">
                        ...
                        <div className="flex items-center gap-2 justify-end pt-4 border-t border-white/5">
                            <span className="text-[7px] md:text-[8px] font-black opacity-30 tracking-widest uppercase font-serif">השראה יומית</span>
                            <div className="px-3 py-1 rounded-full bg-rose-500 text-white font-black text-[8px] md:text-xs shadow-lg">
                       ...
                       <h3 className="text-[10px] md:text-xs font-black text-slate-400 flex items-center gap-1.5 px-2 uppercase tracking-widest font-serif">
                        ...
                        <div className="space-y-3">
                          {announcements.slice(0, 3).map((ann, idx) => (
                              <div key={ann._id || idx} className="bg-gradient-to-br from-white to-purple-50/20 p-5 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border border-purple-100 shadow-sm hover:border-purple-300 transition-all animate-fade-in-up">
                                 <h4 className="font-black text-rose-600 text-[10px] md:text-sm mb-1.5 flex items-center gap-1.5 font-serif">
                                    <Bell size={12}/> {ann.title}
                                 </h4>
                                 <p className="text-[10px] md:text-xs text-slate-500 leading-normal font-medium opacity-90">{ann.content}</p>
                              </div>
                          ))}
                        </div>
                    </div>
                ...
                <div className="bg-white/50 backdrop-blur-md p-6 md:p-8 rounded-[2rem] shadow-sm border border-rose-50 text-center space-y-4 flex flex-col items-center">
                    ...
                    <a href="tel:0500000000" className="block w-full py-3 bg-white text-slate-900 rounded-xl md:rounded-2xl font-black text-[9px] md:text-xs border border-rose-100 shadow-sm hover:bg-rose-50 transition-colors font-serif">חיוג למשרד המעגל</a>
                </div>
            </div>
        </div>
        ...
        <footer className="pt-12 pb-8 border-t border-rose-50 text-center space-y-4">
            ...
            <p className="text-[10px] md:text-xs font-medium text-slate-300 tracking-wide">
                      ...
                      <form onSubmit={handleMembershipSubmit} className="space-y-3 md:space-y-5 pt-4 border-t border-rose-50">
                          ...
                          <div className="flex flex-col gap-2 py-2">
                             <div className="flex items-center gap-2">
                                ...
                                <label htmlFor="terms" className="text-[10px] md:text-xs font-bold text-slate-600">קראתי ואני מאשרת את תקנון האתר</label>
                             </div>
                             <button type="button" onClick={() => setShowTermsModal(true)} className="text-[9px] md:text-[10px] text-purple-500 font-black underline w-fit">לחצי לקריאת התקנון המלא</button>
                          </div>
                          <button type="submit" className="w-full py-3.5 md:py-5 bg-rose-500 text-white rounded-xl md:rounded-2xl font-black text-[10px] md:text-sm shadow-xl hover:bg-purple-600 transition-all active:scale-95 flex items-center justify-center gap-2 mt-2 font-serif">
