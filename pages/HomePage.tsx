      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-20 relative z-10 space-y-8">
        ...
        <section className="relative h-[350px] md:h-[500px] w-full overflow-hidden rounded-[4rem] shadow-2xl border-4 border-white/50">
            ...
            <div key={event.id} className={`absolute inset-0 transition-all duration-1000 ${index === currentSlide ? 'opacity-100 scale-100' : 'opacity-0 scale-110'}`}>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-10 md:p-16 text-right">
                    ...
                    <p className="text-sm md:text-xl text-white/90 font-bold mb-8 flex items-center gap-3 md:justify-start justify-center">
        ...
        <div className="grid lg:grid-cols-3 gap-10">
                    ...
                    <section className="bg-white/90 backdrop-blur-md p-8 md:p-12 rounded-[4rem] shadow-2xl border border-white/50 relative group overflow-hidden">
                        ...
                        <div className="flex flex-col md:flex-row items-center gap-10">
                            <div className="relative">
                                <img src={personality.image} className="w-40 h-40 md:w-56 md:h-56 rounded-[3rem] object-cover shadow-2xl border-4 border-white rotate-3 group-hover:rotate-0 transition-transform duration-700" />
                                <div className="absolute -bottom-3 -right-3 bg-yellow-400 p-4 rounded-2xl shadow-xl animate-bounce"><Sparkles className="text-white" size={24}/></div>
                            </div>
                            <div className="text-center md:text-right flex-1 space-y-3">
                                <span className="text-xs font-black text-rose-500 uppercase tracking-[0.2em] bg-rose-50 px-5 py-2 rounded-full">אשת השבוע שלנו</span>
                                <h3 className="text-4xl md:text-6xl font-black text-slate-900 leading-none">{personality.name}</h3>
                                <p className="text-lg md:text-2xl text-slate-500 font-bold">{personality.role}</p>
                                <button onClick={() => setShowFullInterview(true)} 
                                        className="mt-6 w-full md:w-auto px-10 py-5 bg-slate-900 text-white rounded-[1.5rem] font-black text-base hover:bg-rose-600 transition-all shadow-2xl">
                                    לראיון המלא והמרגש
                                </button>
                            </div>
                        </div>
                        ...
                        {showFullInterview && (
                            <div className="fixed inset-0 z-[200] bg-white/95 backdrop-blur-xl overflow-y-auto animate-slide-up no-scrollbar">
                                <div className="sticky top-0 bg-white/90 backdrop-blur-md p-6 flex justify-between items-center border-b border-slate-100 z-50">
                                    <button onClick={() => setShowFullInterview(false)} className="p-4 bg-slate-100 rounded-full hover:bg-rose-100 transition-colors shadow-sm"><X size={24}/></button>
                                    <h4 className="font-black text-rose-600 text-xl tracking-tight">ראיון השבוע בלעדי</h4>
                                </div>
                                <div className="p-10 md:p-24 max-w-4xl mx-auto space-y-16">
                                    <div className="text-center space-y-8">
                                        <img src={personality.image} className="w-56 h-56 md:w-80 md:h-80 rounded-[4rem] mx-auto object-cover shadow-2xl border-8 border-rose-50 rotate-3" />
                                        <h2 className="text-5xl md:text-8xl font-black text-slate-900 leading-none">{personality.name}</h2>
                                        <div className="h-2 w-32 bg-rose-500 mx-auto rounded-full shadow-lg"></div>
                                    </div>
                                    <div className="space-y-12">
                                        {personality.questions?.map((q: any, i: number) => (
                                            <div key={i} className="bg-[#fafafa]/80 backdrop-blur-sm p-10 md:p-14 rounded-[4rem] border-r-[12px] border-rose-500 shadow-sm animate-fade-in-up" style={{animationDelay: `${i*0.15}s`}}>
                                                <h5 className="font-black text-rose-600 mb-6 text-lg md:text-2xl leading-tight">Q: {q.question}</h5>
                                                <p className="text-slate-800 leading-relaxed text-base md:text-3xl font-bold italic">{q.answer}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
            ...
            <div className="space-y-10">
                ...
                {!user ? (
                   <div onClick={onOpenLogin} className="cursor-pointer bg-gradient-to-br from-rose-600/90 to-pink-600/90 backdrop-blur-md rounded-[4rem] p-10 text-white shadow-2xl shadow-rose-200/50 text-center space-y-6 hover:scale-[1.03] transition-transform border border-white/20">
                      <HeartHandshake size={60} className="mx-auto drop-shadow-lg" />
                      <h3 className="text-3xl font-black">הצטרפי למעגל</h3>
                      <p className="text-sm font-bold opacity-90 leading-relaxed">תהיי חלק מהשינוי בעיר. תרמי לקהילה ותקבלי עולם שלם של תרבות והטבות.</p>
                  ...
                  <div className="bg-white/90 backdrop-blur-md p-10 rounded-[4rem] shadow-2xl border border-white/50 text-center space-y-6">
                      ...
                      <p className="text-sm text-slate-500 font-bold leading-relaxed">לכל שאלה, הצעה או שיתוף פעולה - הצוות שלנו זמין עבורך תמיד.</p>
                  ...
                  <div className="text-right space-y-8">
                      ...
                      <form onSubmit={handleMembershipSubmit} className="space-y-5 pt-8 border-t border-slate-100">
                          <div className="grid grid-cols-2 gap-5">
                            <input required type="number" placeholder="גיל" className="p-5 bg-slate-50 rounded-2xl font-bold outline-none focus:ring-4 focus:ring-rose-100 transition-all" value={membershipForm.age} onChange={e=>setMembershipForm({...membershipForm, age: e.target.value})}/>
                            <input required type="text" placeholder="עיסוק" className="p-5 bg-slate-50 rounded-2xl font-bold outline-none focus:ring-4 focus:ring-rose-100 transition-all" value={membershipForm.occupation} onChange={e=>setMembershipForm({...membershipForm, occupation: e.target.value})}/>
                          </div>
                          <input required type="text" placeholder="כתובת מגורים מלאה" className="w-full p-5 bg-slate-50 rounded-2xl font-bold outline-none focus:ring-4 focus:ring-rose-100 transition-all" value={membershipForm.address} onChange={e=>setMembershipForm({...membershipForm, address: e.target.value})}/>
                          <input required type="tel" placeholder="טלפון ליצירת קשר" className="w-full p-5 bg-slate-50 rounded-2xl font-bold outline-none focus:ring-4 focus:ring-rose-100 transition-all" value={membershipForm.phone} onChange={e=>setMembershipForm({...membershipForm, phone: e.target.value})}/>
